...
public class CharacterOffCameraPositionService {
    ...
    private const float SEARCH_STEP = 0.5f;
    ...
    public bool TryGet(CharacterView view,
        ...
        Vector3 preferred = anchor - _camera.transform.position;
        ...
        float minimum = Mathf.Max(0f, config.SearchDistance.Min);
        float maximum = Mathf.Max(minimum, config.SearchDistance.Max);
        ...
        for (float distance = minimum; distance <= maximum; distance += SEARCH_STEP) {
            for (int i = 0; i < DIRECTION_COUNT; i++) {
                ...
                if (!_candidateService.TryGet(config,
                                              candidate,
                                              anchor,
                                              direction,
    ...
    private Vector3 Rotate(Vector3 direction, int index) {
